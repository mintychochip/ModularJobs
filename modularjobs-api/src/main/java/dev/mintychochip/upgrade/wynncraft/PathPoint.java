...
/**
 * A path point with optional connector type for visual rendering. Path points represent
 * intermediate points in skill tree connections.
 *
 * @param position the grid position (x, y coordinates)
 * @param type the connector type (line, dashed, curve, etc.)
 */
public record PathPoint(@NotNull Position position, @NotNull PathType type) {
  ...
  /** Column coordinate of the path point. */
  public int column() {
    return position.x();
  }

  /** Row coordinate of the path point. */
  public int row() {
    return position.y();
  }
}
