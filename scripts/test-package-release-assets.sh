...
for path, version in ((valid_jar, "2.0.0"), (bad_jar, "1.2.0")):
...
bash "$PACKAGER" 2.0.0 "$VALID_JAR" "$SQL" "$VALID_OUTPUT"
...
if bash "$PACKAGER" 2.0.0 "$BAD_JAR" "$SQL" "$WORK_DIR/bad-version-output"; then
    echo "expected embedded version mismatch to fail" >&2
    exit 1
fi

if bash "$PACKAGER" 2.0 "$VALID_JAR" "$SQL" "$WORK_DIR/bad-semver-output"; then
...
if bash "$PACKAGER" 2.0.0 "$VALID_JAR" "$SQL" "$NONEMPTY_OUTPUT"; then
