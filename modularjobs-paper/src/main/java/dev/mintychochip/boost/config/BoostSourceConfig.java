...
public record BoostSourceConfig(
  ...
  /** Policy config. */
  public record PolicyConfig(@NotNull String type, @Nullable Integer k) {}

  /**
   * A single conditional boost rule: when {@code conditions} hold, {@code boost} applies; higher
   * {@code priority} wins among matching rules in the same source.
   */
  ...
  /**
   * Union-shaped condition descriptor: {@code type} selects the condition kind, and the remaining
   * fields carry that kind's parameters (operator, value(s), nested conditions, resource/effect,
   * amplifiers, min/max, touching, etc.).
   */
