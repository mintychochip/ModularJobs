 ...
 * sender and applies the imported tasks asynchronously, reporting results back on the main thread.
 */
                ...
                .executes(
                    context -> {
                      ...
                      if (!(sender instanceof Player player)) {
                        Messages.send(
                            sender, "<error>This command can only be executed by players");
                      ...
                      editorService
                          .importTasks(code, player.getUniqueId())
                          .thenAccept(
                              result -> {
                                    ...
                                    .runTask(
                                        ...
                                        () -> {
                                          if (result.errors().isEmpty()) {
                                            Messages.send(
                                                player, "<success>Successfully applied edits!");
                                            Messages.send(
                                                player,
                                                "<accent>Tasks imported: "
                                                    + result.tasksImported());
                                            Messages.send(
                                                player,
                                                "<accent>Tasks deleted: " + result.tasksDeleted());
                                          } else {
                                            Messages.send(
                                                player, "<primary>Edits applied with errors:");
                                            Messages.send(
                                                player,
                                                "<accent>Tasks imported: "
                                                    + result.tasksImported());
