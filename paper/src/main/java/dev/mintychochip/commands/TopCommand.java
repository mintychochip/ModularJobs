    ...
    return Commands.literal("top")
        ...
        .then(
            Commands.literal("mode")
                .then(
                    Commands.argument("displayMode", StringArgumentType.string())
                        .suggests(
                            (context, builder) -> {
                        ...
                        .executes(
                            context -> {
                              ...
                              if (!(sender instanceof Player player)) {
                                Messages.send(
                                    sender, "<error>This command can only be used by players.");
                                return 0;
                              }
                              String mode = context.getArgument("displayMode", String.class);
                              if (!mode.equalsIgnoreCase("chat")
                                  && !mode.equalsIgnoreCase("scoreboard")) {
                                Messages.send(
                                    sender,
                                    "<error>Invalid display mode. Use 'chat' or 'scoreboard'.");
                                return 0;
                              }
                              player.setMetadata(
                                  "jobs_display_mode",
                                  new FixedMetadataValue(plugin, mode.toLowerCase()));
                              Messages.send(
                                  sender,
                                  "<success>Display mode set to <secondary>" + mode.toLowerCase());
                              return 1;
                            })))
        .then(
                ...
                .suggests(
                    (context, builder) -> {
                      jobService.getJobs().stream()
                          .map(job -> job.getPlainName().toLowerCase(Locale.ENGLISH))
                          .forEach(builder::suggest);
                      return builder.buildFuture();
                    })
                .then(
                    Commands.argument("pageNumber", IntegerArgumentType.integer())
                        .executes(
                            context -> {
                              CommandSourceStack source = context.getSource();
                              CommandSender sender = source.getSender();
                              ...
                              if (sender instanceof Player player) {
                                ...
                                if ("scoreboard".equalsIgnoreCase(displayMode)) {
                                  TextScoreboard scoreboard =
                                      TextScoreboard.create(
                                          surfaces, Component.text("Job Top - " + jobKey));
                                  ScoreboardJobsTopPageConsumerImpl consumer =
                                      new ScoreboardJobsTopPageConsumerImpl(scoreboard);
                                  int maxPages =
                                      Math.max(1, (ENTRIES_PER_QUERY + PAGE_SIZE - 1) / PAGE_SIZE);
                                  consumer.consume(
                                      Component.text(jobKey),
                                      resultProvider.getPage(key, page, PAGE_SIZE),
                                      sender,
                                      maxPages,
                                      resultProvider.getAllEntries(key));
                                  return 1;
                                }
                              }
                              ChatJobsTopPageConsumerImpl consumer =
                                  new ChatJobsTopPageConsumerImpl();
                              int maxPages =
                                  Math.max(1, (ENTRIES_PER_QUERY + PAGE_SIZE - 1) / PAGE_SIZE);
                              consumer.consume(
                                  Component.text(jobKey),
