...
/**
 * A {@link BoostSource} that resolves its output through a set of
 * {@link Rule rules}. For a given evaluation context, the matching rule with
 * the highest {@link Rule#priority() priority} determines the applied boost;
 * if no rule matches, the source yields no boost.
 */
